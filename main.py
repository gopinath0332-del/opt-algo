...
def run_strategy(config, logger, strategy_config=None):
    """Execute a strategy once.

    Args:
        config: Configuration instance
        logger: Logger instance
        strategy_config: Specific StrategyConfig instance
    """
    from api.rest_client import DeltaRestClient
    from notifications.manager import NotificationManager
    from strategy.short_straddle import ShortStraddleStrategy

    strat = strategy_config or config.strategy
    ...
    lot_size_str = (
        f"{strat.lot_size} (static)"
        if strat.lot_size
        else f"Dynamic ({strat.capital_allocation_pct:.0f}% of balance)"
    )
    startup_msg = (
        f"Strategy: \u001b[1;37m{strat.name}\u001b[0m\n"
        f"Underlying: \u001b[1;37m{strat.underlying}\u001b[0m\n"
        ...
        f"Leverage: \u001b[0;35m{strat.leverage}x\u001b[0m\n"
        f"SL: \u001b[0;33m{f'{strat.stop_loss.value}% of premium' if strat.stop_loss else 'None (Disabled)'}\u001b[0m\n"
        f"Entry: \u001b[0;36m{strat.entry_time} IST\u001b[0m\n"
        f"Exit: \u001b[0;36m{strat.exit_time} IST\u001b[0m\n"
        f"Time: {now.strftime('%Y-%m-%d %H:%M:%S IST')}"
    )

    notifier.send_status_message(
        f"🚀 Options Bot Starting — {strat.underlying}",
        startup_msg,
...
def check_and_resume_active_trades(config, logger):
    ...
    for strat in config.get_straddle_strategies():
        entry_time = strat.entry_time
        exit_time = strat.exit_time
        try:
            entry_h, entry_m = map(int, entry_time.split(":"))
            exit_h, exit_m = map(int, exit_time.split(":"))
        except ValueError as e:
            logger.error(f"Invalid entry/exit time format in configuration for {strat.underlying}: {e}")
            continue
        ...
        if today_entry <= now < today_exit:
            logger.info(f"Service started/restarted within trading window for {strat.underlying}. Performing recovery checks...")
            try:
                ...
                if config.firestore_enabled:
                    ...
                    open_trades = get_open_trades(strat.underlying)
                    if open_trades:
                        from api.rest_client import DeltaRestClient
                        client = DeltaRestClient(config)
                        try:
                            ...
                            logger.error(f"Failed to fetch positions from exchange during recovery for {strat.underlying}: {e}")
                            positions = []
                        ...
                        for trade in open_trades:
                            ...
                            if call_pid and put_pid:
                                ...
                                if has_call and has_put:
                                    logger.info(f"Found active open trade in Firestore (ID: {trade.get('trade_id')}) for {strat.underlying} with matching positions on Delta Exchange.")
                                    recovered_state = {
                                    ...
                                    logger.warning(f"Trade {trade.get('trade_id')} for {strat.underlying} is marked OPEN in Firestore but matching positions are not active on Delta Exchange.")

                # 2. Fallback Recovery: Delta Exchange Positions directly
                if not recovered_state:
                    logger.info(f"Firestore recovery did not find active trade for {strat.underlying}. Checking Delta Exchange positions directly...")
                    from api.rest_client import DeltaRestClient
                    client = DeltaRestClient(config)
                    try:
                        positions = client.get_positions()
                    ...
                    for pos in positions:
                        ...
                        if size < 0:  # Short position
                            ...
                            if (symbol.startswith("C-") or symbol.startswith("P-")) and strat.underlying in symbol:
                        ...
                        if len(opts) >= 2:
                            ...
                            if calls and puts:
                                ...
                                if call_opt["size"] == put_opt["size"]:
                                    logger.info(f"Recovered open straddle from exchange for {strat.underlying}: Call={call_opt['symbol']}, Put={put_opt['symbol']}, size={call_opt['size']}")
                                    parts = suffix.split("-")
                                    strike = 0.0
                                    if len(parts) >= 2:
                                        try:
                                    ...
                                    trade_id = f"recovered_straddle_{strat.underlying}_{now.strftime('%Y%m%d_%H%M%S')}"
                                    ...
                                    recovered_state = {
                                        ...
                                        "sl_threshold": entry_prem * (strat.stop_loss.value / 100.0) if strat.stop_loss else None,
                                        "atm_strike": strike,
                                        "spot_price": strike,
                ...
                if recovered_state:
                    logger.info(f"Resuming options trade monitoring for {strat.underlying} trade {recovered_state['trade_id']}")
                    from api.rest_client import DeltaRestClient
                    from notifications.manager import NotificationManager
                    from strategy.short_straddle import ShortStraddleStrategy
                    ...
                    logger.info(f"No active options trades detected to resume for {strat.underlying}.")
            except Exception as e:
                logger.error(f"Error during recovery for {strat.underlying}: {e}")
...
def main():
    ...
    try:
        ...
        straddles = config.get_straddle_strategies()

        if args.once:
            # Run immediately for all configured straddles
            logger.info("Running strategies once (--once mode)")
            for strat in straddles:
                run_strategy(config, logger, strategy_config=strat)
        else:
            # Run on daily schedule
            import schedule

            for strat in straddles:
                logger.info(f"Scheduling strategy '{strat.name}' ({strat.underlying}) to run daily at {strat.entry_time} IST")
                ...
                def make_scheduled_runner(s_cfg):
                    def scheduled_run():
                        ...
                        try:
                            ...
                            logger.error(f"Scheduled strategy run for {s_cfg.underlying} failed: {e}", exc_info=True)
                    return scheduled_run

                schedule.every().day.at(strat.entry_time).do(make_scheduled_runner(strat))
            ...
            sched_details = "\n".join(
                f"• \u001b[1;37m{s.underlying}\u001b[0m: Entry \u001b[0;36m{s.entry_time} IST\u001b[0m -> Exit \u001b[0;36m{s.exit_time} IST\u001b[0m"
                for s in straddles
            )
            startup_msg = (
                f"```ansi\n"
                ...
                f"```\n"
                f"*Service is online and waiting for scheduled execution times.*"
            )
            ...
            logger.info("Scheduler active — waiting for scheduled runs. Press Ctrl+C to stop.")
