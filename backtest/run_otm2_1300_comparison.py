"""
backtest/run_otm2_1300_comparison.py
======================================
Backtest & Comparison:
  Baseline Strategy : Current Config (17:00 IST / 11:30 UTC entry, ATM Short Straddle, Expiry exit, No SL)
  New Idea Strategy : 13:00 IST / 07:30 UTC entry, OTM+2 Short Strangle (Call=ATM+400, Put=ATM-400), Expiry exit, No SL

Usage:
  python backtest/run_otm2_1300_comparison.py
  python backtest/run_otm2_1300_comparison.py --month 2025-01
  python backtest/run_otm2_1300_comparison.py --start 2025-01 --end 2026-06
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Backtest & Compare 13:00 IST OTM+2 Strangle vs Current Config (17:00 IST ATM Straddle)",
    ...
    p.add_argument("--otm-steps", type=int,   default=2,
                   help="Number of strike steps OTM for new idea (default: 2 = $400 away from ATM)")
...
def print_comparison(current_port: Portfolio, new_port: Portfolio, otm_steps: int) -> None:
    """Print a side-by-side comparison table of Current Config vs New Idea."""
    ...
    print()
    print("=" * 80)
    print("  STRATEGY COMPARISON: Current Configuration  vs  New Idea")
    print(f"  Current Config : Entry 17:00 IST (11:30 UTC), ATM Straddle, Expiry Exit, No SL")
    print(f"  New Idea       : Entry 13:00 IST (07:30 UTC), OTM+{otm_steps} Strangle (+/-${otm_distance}), Expiry Exit, No SL")
    print("=" * 80)
    print(f"  {'Metric':<30}  {'Current Config (17:00 ATM)':>{col}}  {f'New Idea (13:00 OTM+{otm_steps})':>{col}}")
    print(sep)
...
def main() -> None:
    ...
    # New Idea Strategy: 13:00 IST entry, OTM+2 Strangle (otm_steps=2), Expiry Exit (12:00 UTC)
    ...
    log.info("=" * 60)
    log.info("  Backtest & Compare: Current Config vs New Idea")
    ...
    log.info("  New Idea    : Entry 13:00 IST (07:30 UTC) | OTM+%d Strangle (+$%d/-$%d) | Expiry Exit | No SL",
             args.otm_steps, args.otm_steps * 200, args.otm_steps * 200)
    log.info("=" * 60)

    current_engine = ShortStraddleEngine(current_cfg)
    new_engine     = ShortStrangleEngine(new_cfg)

    log.info("Running backtests on all trading days...")
    day_count = 0

    # Since iter_trading_days uses cfg.start_month and cfg.end_month, we can iterate over one cfg
    ...
    print_comparison(current_port, new_port, args.otm_steps)

    print("---- Current Config (17:00 IST ATM Straddle) ----")
    current_port.print_summary()

    print()
    print(f"---- New Idea (13:00 IST OTM+{args.otm_steps} Strangle) ----")
    ...
    if new_engine.trades:
        n_dir  = REPORTS_DIR / f"new_idea_1300_otm{args.otm_steps}_{start}_{end}_{ts_str}"
