"""
backtest/run_1715_entry_comparison.py
======================================
Backtest & Comparison:
  Baseline Strategy : Current Config (17:00 IST / 11:30 UTC entry, ATM Short Straddle, Expiry exit, No SL)
  New Idea Strategy : 17:15 IST / 11:45 UTC entry, ATM Short Straddle, Expiry exit, No SL

Usage:
  python backtest/run_1715_entry_comparison.py
  python backtest/run_1715_entry_comparison.py --month 2025-01
  python backtest/run_1715_entry_comparison.py --start 2025-01 --end 2026-06
"""
...
ENTRY_1715_UTC = time(11, 45)
EXPIRY_EXIT_UTC = time(12, 0)


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Backtest & Compare 17:00 IST vs 17:15 IST Entry (ATM Short Straddle)",
...
def print_comparison(current_port: Portfolio, new_port: Portfolio) -> None:
    """Print a side-by-side comparison table of Current Config (17:00) vs New Idea (17:15)."""
    ...
    print()
    print("=" * 80)
    print("  STRATEGY COMPARISON: Current Config (17:00)  vs  New Idea (17:15)")
    print(f"  Current Config : Entry 17:00 IST (11:30 UTC), ATM Straddle, Expiry Exit, No SL")
    print(f"  New Idea       : Entry 17:15 IST (11:45 UTC), ATM Straddle, Expiry Exit, No SL")
    print("=" * 80)
    print(f"  {'Metric':<30}  {'Current (17:00 IST)':>{col}}  {'New Idea (17:15 IST)':>{col}}")
    print(sep)
    ...
    row("SL hits",             c.get("sl_hit_count", 0),                 n.get("sl_hit_count", 0))
    row("Avg hold time (min)", f"{c.get('avg_hold_minutes', 0):.1f}",    f"{n.get('avg_hold_minutes', 0):.1f}")
    print("=" * 80)
    print()
...
def main() -> None:
    ...
    # Current Configuration: 17:00 IST entry, ATM Straddle, Expiry Exit
    ...
    # New Idea Strategy: 17:15 IST entry, ATM Straddle, Expiry Exit
    new_cfg = BacktestConfig(
        **base_cfg,
        entry_time_utc = ENTRY_1715_UTC,
        exit_time_utc  = EXPIRY_EXIT_UTC,
        otm_steps      = 0,
    )

    log.info("=" * 60)
    log.info("  Backtest & Compare: 17:00 IST vs 17:15 IST Entry")
    ...
    log.info("  New Idea    : Entry 17:15 IST (11:45 UTC) | ATM Straddle | Expiry Exit | No SL")
    log.info("=" * 60)
    ...
    print()
    print("---- New Idea (17:15 IST ATM Straddle) ----")
    ...
    if new_engine.trades:
        n_dir  = REPORTS_DIR / f"new_idea_1715_atm_{start}_{end}_{ts_str}"
